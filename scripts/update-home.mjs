...
if(!pattern.test(html))throw Error('Missing '+name+' marker');let source=fs.readFileSync(new URL('ui/'+name,root),'utf8');if(name==='home.js')source+='\n'+fs.readFileSync(new URL('ui/homestead.js',root),'utf8')+'\n'+fs.readFileSync(new URL('ui/cafe-room.js',root),'utf8');html=html.replace(pattern,()=>wrap(source));}
