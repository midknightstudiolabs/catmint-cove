...
let shop=make();E.unlock(shop,0);shop.shells=E.shops[1].cost-1;assert.equal(E.upgradeShop(shop,0),false);assert.equal(shop.shells,E.shops[1].cost-1);
shop.shells=150000;const pantry=JSON.stringify(shop.homestead.stock);assert(E.upgradeShop(shop,0));assert.equal(shop.cafe.shopTier,1);assert.equal(shop.shells,150000-E.shops[1].cost);
shop=JSON.parse(JSON.stringify(shop));assert(E.upgradeShop(shop,0));assert.equal(shop.cafe.shopTier,2);assert.equal(shop.shells,150000-E.shops[1].cost-E.shops[2].cost);assert.equal(JSON.stringify(shop.homestead.stock),pantry);assert.equal(E.upgradeShop(shop,0),false);
console.log('PASS: three sequential shop stages, exact 20k/45k costs, insufficient funds, final cap and save preservation.');
