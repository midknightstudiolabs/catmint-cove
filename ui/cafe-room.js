...
function neoPaintCafe(cv,h){
  ...
  toolbar.querySelector('[data-edit]').onclick=()=>{editing=!editing;selected=null;ghost=null;controls();status.textContent=editing?'Select furniture, then tap a free tile or drag. Keep the door and walkways clear.':'Your café layout is saved.';save();};
  ...
  function paint(t){
    ...
    for(const f of items.filter(f=>!f.boxed))layers.push({y:center(f).y,draw:()=>{
      ...
      else {g.fillStyle=f.type==='oven'?'#829b91':'#b99b71';g.fillRect(-32,-28,64,48);g.fillStyle='#faf1da';g.fillRect(-32,-30,64,12);g.fillStyle='#43543f';if(f.type==='oven'){g.fillRect(-21,-5,42,19);g.fillStyle=h.stoves.some(Boolean)?'#d6af6a':'#a4b6ad';g.fillRect(-15,0,30,10);}else {g.fillStyle='#fff8e8';g.fillRect(-22,-24,44,9);}g.fillStyle='#43543f';g.font='11px sans-serif';g.fillText(f.type==='oven'?(h.stoves.some(Boolean)?'Cooking':'Stove'):'Counter',0,36);}
      if(selected===f){g.strokeStyle='#477354';g.lineWidth=3;g.strokeRect(-37,-37,74,74);}g.restore();}});
    if(h.open&&h.counter&&!editing){const tables=items.filter(f=>f.type==='table'&&!f.boxed);const i=h.served%Math.max(1,tables.length),table=tables[i],c=guests[h.served%Math.max(1,guests.length)];if(table&&c){const path=neoCafeRoute(items,table);if(path){const phase=Math.max(0,Math.min(.999,(Date.now()-h.lastSale)/60000));const distance=still?1:phase<.18?phase/.18:phase>.86?(1-phase)/.14:1;const k=distance*(path.length-1),a=center(path[Math.floor(k)]),b=center(path[Math.min(path.length-1,Math.ceil(k))]),f=k%1,p={x:a.x+(b.x-a.x)*f,y:a.y+(b.y-a.y)*f};layers.push({y:p.y,draw:()=>{g.save();g.translate(p.x,p.y+12);g.scale(1.65,1.65);c.state=distance<1?'walking':'loafing';c.walk=still?0:t/130;c.walkAmt=distance<1?1:0;c.face=b.x<a.x?-1:1;c.bob=0;drawCat(g,c,true);g.restore();g.fillStyle='#43543f';g.font='12px sans-serif';g.textAlign='center';g.fillText(phase<.18?'Finding a seat':phase<.3?'Being served':phase<.86?'Enjoying a treat':'Thank you · 2 shells',p.x,p.y-45);}});}}}
