...
function neoPaintCafe(cv,h){
  ...
  function paint(t){
    ...
    for(const f of items.filter(f=>!f.boxed))layers.push({y:center(f).y,draw:()=>{
      ...
      else {g.fillStyle=f.type==='oven'?'#829b91':'#b99b71';g.fillRect(-32,-28,64,48);g.fillStyle='#faf1da';g.fillRect(-32,-30,64,12);g.fillStyle='#43543f';if(f.type==='oven'){g.fillRect(-21,-5,42,19);g.fillStyle=h.batch?'#d6af6a':'#a4b6ad';g.fillRect(-15,0,30,10);}else {g.fillStyle='#fff8e8';g.fillRect(-22,-24,44,9);}g.fillStyle='#43543f';g.font='11px sans-serif';g.fillText(f.type==='oven'?(h.batch?'Cooking':'Stove'):'Counter',0,36);}
      if(selected===f){g.strokeStyle='#477354';g.lineWidth=3;g.strokeRect(-37,-37,74,74);}g.restore();}});
    if(h.counter&&!editing){const tables=items.filter(f=>f.type==='table'&&!f.boxed);tables.forEach((table,i)=>{const c=guests[i];if(!c)return;const path=neoCafeRoute(items,table);if(!path)return;const phase=still?.5:((t/1000+i*9)%36)/36;let distance=phase<.22?phase/.22:phase>.82?(1-phase)/.18:1;distance=Math.max(0,Math.min(1,distance));const k=distance*(path.length-1),a=center(path[Math.floor(k)]),b=center(path[Math.min(path.length-1,Math.ceil(k))]),f=k%1,p={x:a.x+(b.x-a.x)*f,y:a.y+(b.y-a.y)*f};layers.push({y:p.y,draw:()=>{g.save();g.translate(p.x,p.y+12);g.scale(1.65,1.65);c.state=distance<1?'walking':'loafing';c.walk=still?0:t/130;c.walkAmt=distance<1?1:0;c.face=b.x<a.x?-1:1;c.bob=still?0:Math.sin(t/800+i)*.3;drawCat(g,c,true);g.restore();}});});}
