(function(root){
 ...
 root.CoveGardenScene={stop(){stop();},mount(host,actors,drawCat){
  ...
  function paint(t){if(ended)return;if(!host.isConnected||host.closest('[hidden]')){stop();return;}frame=requestAnimationFrame(paint);if(document.hidden||!visible){last=0;return;}const dt=last?Math.min(.05,(t-last)/1000):0;last=t;if(!reduced.matches)clock+=dt;
   ...
   cats.forEach((cat,i)=>{
    const y=lanes[Math.min(i+1,lanes.length-1)],span=Math.max(0,w-70);
    // A gentle stroll at a consistent scale on phones and tablets, then a rest.
    const travel=Math.max(8,span/(23+i*2)),period=travel+3+i;
    const time=clock+i*8,u=time%period,p=Math.min(1,u/travel);
    const phase=p*p*(3-2*p),forward=Math.floor(time/period)%2===0;
    const x=35+span*(forward?phase:1-phase);
    const speed=reduced.matches?0:span*6*p*(1-p)/travel;
    const amount=Math.min(1,speed/20);
    cat.walk=(cat.walk||0)+speed*dt/.66*.34;
    cat.walkAmt=(cat.walkAmt||0)+(amount-(cat.walkAmt||0))*(1-Math.exp(-dt*10));
    drawCat(ctx,cat,x,y+9,.66,clock*1000,amount>.01,forward?1:-1);
   });
