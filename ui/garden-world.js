...
const cats=actors.slice(0,3).map((cat,i)=>({cat,x:.18+i*.3,from:0,to:0,wait:.3+i*.8,elapsed:0,duration:0,face:i%2?-1:1,visits:0}));
...
function paint(t){if(ended)return;if(!host.isConnected){stop();return;}frame=requestAnimationFrame(paint);if(document.hidden||host.closest('[hidden]')||!visible){last=0;return;}const dt=last?Math.min(.05,(t-last)/1000):0;last=t;if(!reduced.matches)clock+=dt;
ctx.clearRect(0,0,w,h);ctx.drawImage(land,0,0,w,h);
cats.forEach((a,i)=>{const span=Math.max(1,w-70),before=a.x;
{if(a.wait>0){a.wait-=dt;if(a.wait<=0){a.visits++;a.from=a.x;const target=.12+((a.visits*.381966+i*.29)%1)*.76;a.to=Math.abs(target-a.x)<.12?Math.max(.1,Math.min(.9,a.x+(a.x>.5?-.2:.2))):target;a.duration=Math.max(3,Math.abs(a.to-a.x)*span/(reduced.matches?9:22));a.elapsed=0;a.face=a.to>a.x?1:-1;}}else{a.elapsed=Math.min(a.duration,a.elapsed+dt);const p=a.elapsed/a.duration;a.x=a.from+(a.to-a.from)*p*p*(3-2*p);if(p===1)a.wait=3+(a.visits+i)%4*1.6;}}
const speed=dt?Math.abs(a.x-before)*span/dt:0,cat=a.cat;cat.walk=(cat.walk||0)+speed*dt/.72*.34;cat.walkAmt=(cat.walkAmt||0)+((reduced.matches?0:Math.min(1,speed/18))-(cat.walkAmt||0))*(1-Math.exp(-dt*10));cat.gardenRest=!reduced.matches&&a.wait>0&&a.visits%3===1?'grooming':'loafing';const y=lanes[(i+1)%lanes.length];drawCat(ctx,cat,35+a.x*span,y+9,.72,clock*1000,speed>.1,a.face);});
