(function(root){
 ...
 root.CoveCafeView={open(a){
  ...
  panel.hidden=false;let previewFinish=null,confirmFinish=null,previewTier=null,confirmShop=false;let workshop=false,recipeResult=null,lessonStep=0;let tasting='',previewEquipment=null,displayState=s;let view='outside',tab=null,layout={k:0,cx0:0,offset:0},frame=0,last=0,timer;const actors=a.actors();stopActive=()=>{cancelAnimationFrame(frame);clearInterval(timer);};
  ...
  function render(){
   ...
   panel.classList.toggle('cc-locked',!s.unlocked);const needs=new Set(s.menu.length?E.recipes.filter(r=>s.menu.includes(r.id)).flatMap(r=>Object.keys(r.inputs)):['coffee','catmint','honey']);const rdy=ready(),ripeChip=rdy.count?`<button class="cc-pip cc-ripe" data-ripe aria-label="${rdy.count} patches ripe in the Garden"><span>${ripeLabel(rdy.count)}</span></button>`:'';const pips=ripeChip+[...needs].map(k=>{const n=g.homestead.stock[k]||0,low=n<(E.recipes.filter(r=>s.menu.includes(r.id)).reduce((m,r)=>Math.max(m,r.inputs[k]||0),1));return `<button class="cc-pip${low?' low':''}" data-pip="${k}" aria-label="${E.ingredients[k].name}: ${n} in pantry${low?', running low':''}"><span>${E.ingredients[k].name}</span><b>${n}</b></button>`;}).join('');const anyLow=s.unlocked&&pips.includes(' low');panel.innerHTML=`<canvas aria-label="${view==='inside'?'Behind the counter, customers ordering at the window':'Seaside café kiosk'}" width="720" height="380"></canvas><div class="cc-top"><header class="cc-head"><button class="btn cc-back" data-close aria-label="Return to Cove">‹ Cove</button><h2>Catmint Café</h2><span class="cc-head-end"></span></header>${s.unlocked?`<div class="cc-live"><div class="cc-live-row"><b class="cc-shells">${Math.floor(g.shells).toLocaleString()} Shells</b><span>${s.served} served</span></div><div class="cc-pips" role="group" aria-label="Pantry stock">${pips}</div><p class="cc-status" role="status"></p></div>`:`<div class="cc-live cc-live-lite"><b class="cc-shells">${Math.floor(g.shells).toLocaleString()} Shells</b></div>`}</div><div class="cc-switch" aria-label="Café view"><button aria-pressed="${view==='outside'}" data-view="outside">Outside</button><button aria-pressed="${view==='inside'}" data-view="inside">Inside</button></div>`;
   ...
   if(!s.unlocked){panel.insertAdjacentHTML('beforeend',`<div class="cc-intro"><h3>A little coffee. A lot of company.</h3><p>Your cats run every order. Set up your café and it comes with 6 coffee beans, 6 catmint and 2 honey.</p><button class="btn primary" data-start ${g.shells<120?'disabled':''}>Set up your café · 120 Shells</button><small>${g.shells<120?'Save '+(120-Math.floor(g.shells))+' more Shells to get started.':'Make a drink, put it on the menu, then open for guests.'}</small></div>`);panel.querySelector('[data-start]').onclick=()=>commit(()=>{E.unlock(g,Date.now());if(guideOn){tab='menu';view='inside';}});}
    ...
    panel.insertAdjacentHTML('beforeend',`<nav class="cc-tabs" aria-label="Café management">${['menu','pantry','report','upgrades'].map(k=>`<button aria-pressed="${tab===k}" data-tab="${k}">${k[0].toUpperCase()+k.slice(1)}${k==='pantry'&&anyLow?'<i class="cc-dot" aria-label="running low"></i>':''}</button>`).join('')}</nav><div class="cc-content" ${tab?'':'hidden'}></div>`);
