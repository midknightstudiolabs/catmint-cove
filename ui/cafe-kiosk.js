(function(root){
 ...
 root.CoveCafeView={open(a){
  ...
  panel.hidden=false;let previewFinish=null,confirmFinish=null,previewTier=null,confirmShop=false,previewDecor=null,confirmDecor=null;const floats=[];let workshop=false,recipeResult=null,lessonStep=0;let tasting='',previewEquipment=null,displayState=s;let view='outside',tab=null,layout={k:0,cx0:0,offset:0},frame=0,last=0,timer;const actors=a.actors();stopActive=()=>{cancelAnimationFrame(frame);clearInterval(timer);};
  ...
  function render(){
   ...
   panel.classList.toggle('cc-locked',!s.unlocked);const needs=new Set(s.menu.length?E.recipes.filter(r=>s.menu.includes(r.id)).flatMap(r=>Object.keys(r.inputs)):['coffee','catmint','honey']);const goal=s.unlocked?E.nextGoal(g):null,goalHtml=goal?`<button class="cc-goal" data-goal="${goal.id}" aria-label="Next goal: ${goal.label}, ${goal.cost.toLocaleString()} Shells"><span>Next: ${goal.label}</span><i style="--p:${Math.round(Math.min(1,g.shells/goal.cost)*100)}%"></i><b>${Math.floor(Math.min(g.shells,goal.cost)).toLocaleString()} / ${goal.cost.toLocaleString()}</b></button>`:'';const goalAffordable=!!goal&&g.shells>=goal.cost;const rdy=ready(),ripeChip=rdy.count?`<button class="cc-pip cc-ripe" data-ripe aria-label="${rdy.count} patches ripe in the Garden"><span>${ripeLabel(rdy.count)}</span></button>`:'';const pips=ripeChip+[...needs].map(k=>{const n=g.homestead.stock[k]||0,low=n<(E.recipes.filter(r=>s.menu.includes(r.id)).reduce((m,r)=>Math.max(m,r.inputs[k]||0),1));return `<button class="cc-pip${low?' low':''}" data-pip="${k}" aria-label="${E.ingredients[k].name}: ${n} in pantry${low?', running low':''}"><span>${E.ingredients[k].name}</span><b>${n}</b></button>`;}).join('');const anyLow=s.unlocked&&pips.includes(' low');panel.innerHTML=`<canvas aria-label="${view==='inside'?'Behind the counter, customers ordering at the window':'Seaside café kiosk'}" width="720" height="380"></canvas><div class="cc-top"><header class="cc-head"><button class="btn cc-back" data-close aria-label="Return to Cove">‹ Cove</button><h2 class="cc-title"></h2><span class="cc-head-end"></span></header>${s.unlocked?`<div class="cc-live"><div class="cc-live-row"><b class="cc-shells">${Math.floor(g.shells).toLocaleString()} Shells</b><span>${E.rating(s)?'★ '+E.rating(s).toFixed(1)+' · ':''}${s.served} served</span></div><div class="cc-pips" role="group" aria-label="Pantry stock">${pips}</div><p class="cc-status" role="status"></p><div class="cc-cta-row"></div>${goalHtml}</div>`:`<div class="cc-live cc-live-lite"><b class="cc-shells">${Math.floor(g.shells).toLocaleString()} Shells</b></div>`}</div><div class="cc-switch" aria-label="Café view"><button aria-pressed="${view==='outside'}" data-view="outside">Outside</button><button aria-pressed="${view==='inside'}" data-view="inside">Inside</button></div>`;
   ...
   if(!s.unlocked){panel.insertAdjacentHTML('beforeend',`<div class="cc-intro"><h3>A little coffee. A lot of company.</h3><p>Your cats run every order. Set up your café and it comes with 6 coffee beans, 6 catmint and 2 honey.</p><button class="btn primary" data-start ${g.shells<120?'disabled':''}>Set up your café · 120 Shells</button><small>${g.shells<120?'Save '+(120-Math.floor(g.shells))+' more Shells to get started.':'Make a drink, put it on the menu, then open for guests.'}</small></div>`);panel.querySelector('[data-start]').onclick=()=>commit(()=>{E.unlock(g,Date.now());if(guideOn){tab='menu';view='inside';}});}
   else{
    panel.insertAdjacentHTML('beforeend',`<nav class="cc-tabs" aria-label="Café management">${['menu','pantry','report','upgrades'].map(k=>`<button aria-pressed="${tab===k}" data-tab="${k}">${k[0].toUpperCase()+k.slice(1)}${k==='pantry'&&anyLow?'<i class="cc-dot" aria-label="running low"></i>':''}${k==='upgrades'&&goalAffordable?'<i class="cc-dot good" aria-label="something new to buy"></i>':''}</button>`).join('')}</nav><div class="cc-content" ${tab?'':'hidden'}></div>`);
    panel.querySelectorAll('[data-tab]').forEach(b=>b.onclick=()=>{tab=tab===b.dataset.tab?null:b.dataset.tab;render();});const content=panel.querySelector('.cc-content');panel.querySelectorAll('[data-pip]').forEach(b=>b.onclick=()=>{tab='pantry';render();});panel.querySelectorAll('[data-ripe]').forEach(b=>b.onclick=harvestRipe);
    ...
    if(tab==='menu'){
     ...
     if(!workshop){
      const tiles=E.recipes.filter(r=>s.discovered.includes(r.id)).map(r=>{const level=s.recipeLevels?.[r.id]||0,sales=s.sales[r.id]||0,on=s.menu.includes(r.id),short=Object.entries(r.inputs).some(([k,n])=>(g.homestead.stock[k]||0)<n);const state=on?(!s.open?'On menu · café closed':short?'On menu · needs ingredients':'Serving now'):'Not on menu';return `<article class="cc-card cc-recipe${on?' on':''}"><span class="cc-cup">${cup(r.id)}</span><div class="cc-rbody"><h3>${escape(E.displayName(s,r))}</h3><small>${E.price(s,r)} Shells · ${Object.entries(r.inputs).map(([k,n])=>{const nm=E.ingredients[k].name.toLowerCase();return n+' '+(n===1&&nm.endsWith('s')?nm.slice(0,-1):nm);}).join(' + ')}</small><strong class="cc-recipe-state${on&&short?' warn':''}">${state}</strong></div><button class="cc-flip" role="switch" aria-checked="${on}" aria-label="Serve ${escape(E.displayName(s,r))} on the menu" data-recipe="${r.id}"><span class="cc-toggle" aria-hidden="true"></span></button><details class="cc-recipe-details"><summary>${sales} enjoyed · Level ${level+1}/3</summary><button class="btn" data-name="${r.id}">Rename</button><button class="btn" data-improve="${r.id}" ${level>=2||sales<(level+1)*10||g.shells<(level+1)*50?'disabled':''}>${level>=2?'Mastered':sales<(level+1)*10?'Serve '+((level+1)*10-sales)+' more to improve':'Improve · '+((level+1)*50)+' Shells · +2 per sale'}</button></details></article>`;}).join('');content.innerHTML=`${s.lessonComplete?`<p>Cats serve what is on your menu, while ingredients last.</p>${tiles}<button class="btn cc-new" data-create>+ New recipe</button>`:`<p>Cats serve what is on your menu, while ingredients last.</p><button class="btn primary" data-create>Make your first coffee · free lesson</button>`}<button class="btn" data-garden>Grow ingredients in Cove Garden</button>`;
   ...
   if(tab==='upgrades'&&s.unlocked&&view==='outside'){
    ...
    box.querySelector('[data-current]').onclick=()=>{previewTier=null;previewFinish=null;confirmFinish=null;confirmShop=false;previewDecor=null;confirmDecor=null;render();panel.scrollTop=0;};
   ...
   const cv=panel.querySelector('canvas');cv.onclick=e=>{if(tab){tab=null;render();return;}if(view!=='outside')return;const b=cv.getBoundingClientRect(),fit=layout.k||b.width/720,x=(e.clientX-b.left)/fit+layout.cx0,y=(e.clientY-b.top)/fit-layout.offset;if(x>190&&x<530&&y>100&&y<235){view='inside';render();}else if(x>25&&x<120&&y>270&&y<350){panel.querySelector('.cc-sale')?.remove();const hello=document.createElement('div');hello.className='cc-sale';hello.setAttribute('role','status');hello.textContent=['This seat has excellent purr acoustics.','Stay a little. The sea isn’t going anywhere.','A quiet bench. Very important cat business.'][Math.floor(Date.now()/1000)%3];panel.append(hello);setTimeout(()=>hello.remove(),4500);}};
   panel.onkeydown=e=>{if(e.key==='Escape'){if(tab){tab=null;render();}else panel.querySelector('[data-close]').click();}};
   panel.querySelector('.cc-switch').insertAdjacentHTML('afterend','<p class="cc-visit-hint">'+(view==='outside'?'Tap the window to step inside':'A warm cup and a little company.')+'</p>');
   drawGuide();
   const renderedServed=s.served,renderedOpen=s.open,renderedPending=s.pending?.at;status();frame=requestAnimationFrame(paint);timer=setInterval(()=>{if(panel.hidden||!panel.isConnected){clearInterval(timer);cancelAnimationFrame(frame);return;}const before=s.served;E.settle(g,Date.now());if(s.served!==renderedServed){a.save();a.hud();floats.push({t0:performance.now(),text:'+'+(s.lastCompleted?.price||0)});a.sfx?.('collect');render();const notice=document.createElement('div');notice.className='cc-sale';notice.setAttribute('role','status');const sold=E.recipes.find(r=>r.id===s.lastCompleted?.id);notice.textContent=sold?E.displayName(s,sold)+' · +'+(s.lastCompleted.price||E.price(s,sold))+' Shells — '+(s.lastCompleted.response?.text||'A happy little moment.'):'An order enjoyed · Shells added';panel.append(notice);setTimeout(()=>notice.remove(),4000);}else if(s.open!==renderedOpen||s.pending?.at!==renderedPending){a.save();render();}else status();},1000);
  }
  function status(){const el=panel.querySelector('.cc-status');if(!el)return;const n=Math.max(0,Math.ceil((s.cursor+E.interval(s)-Date.now())/1000));el.textContent=!s.unlocked?'A tiny place for your next big idea.':s.pending?'Brewing '+E.displayName(s,E.recipes.find(r=>r.id===s.pending.id))+' · ready in '+Math.max(0,Math.ceil((s.pending.at-Date.now())/1000))+'s':!s.menu.length?'Create a drink and add it to your menu before opening.':!s.open?(s.closedReason==='ingredients'?'Closed · out of ingredients. Restock in Pantry or grow more in the Garden.':'Ready when you are · tap Closed above to open.'):!E.available(s,g.homestead.stock).length?'Taking a nap · stock an ingredient or add a drink to the menu.':`Open · next guest in ${Math.floor(n/60)}:${String(n%60).padStart(2,'0')}`;}
