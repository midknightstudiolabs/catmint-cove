...
let stripOpen=(()=>{try{return localStorage.getItem('neo.cafe.strip')==='1';}catch(e){return false;}})();let lastTab=null,view='outside',tab=null,layout={k:0,cx0:0,offset:0},frame=0,last=0,timer,cam=null,camT=0,camKey='',edgeCv=[null,null],edgeT=0,edgeSig='';const actors=a.actors();stopActive=()=>{cancelAnimationFrame(frame);clearInterval(timer);for(const cv of edgeCv){if(cv){cv.width=1;cv.height=1;}}const cv=panel.querySelector("canvas");if(cv){cv.width=1;cv.height=1;}stopActive=()=>{};};
  ...
  function equipPopup(kind,ax,ay){
   ...
   if(kind==='speed'){
    ...
    go=()=>{commit(()=>{if(g.shells>=cost){g.shells-=cost;s.speed++;}});render();equipPopup('speed');};
    ...
    go=()=>{commit(()=>{if(!s.cookware&&g.shells>=180){g.shells-=180;s.cookware=true;}});render();equipPopup('cookware');};
   }else if(kind==='beans'){
    const cost=E.ingredients.coffee.import*5;
    title='Coffee beans';desc=(g.homestead.stock.coffee||0)+' in the pantry.';
    label='Buy 5 · '+cost+' Shells';need=g.shells>=cost?null:cost;
    go=()=>{commit(()=>{if(g.shells>=cost){g.shells-=cost;E.acquire(g,'coffee',5,cost);}});render();equipPopup('beans');};
   }
  ...
  function render(){
   ...
   panel.querySelectorAll('[data-view]').forEach(b=>b.onclick=()=>{
     if(view===b.dataset.view)return;const next=b.dataset.view,cv=panel.querySelector('canvas'),calm=matchMedia('(prefers-reduced-motion: reduce)').matches;
     const go=()=>{view=next;render();if(!calm){const nc=panel.querySelector('canvas');if(nc){nc.style.transition='none';nc.style.opacity='0';requestAnimationFrame(()=>requestAnimationFrame(()=>{nc.style.transition='opacity .22s ease';nc.style.opacity='1';}));}}};
     if(calm||!cv){go();return;}cv.style.transition='opacity .12s ease';cv.style.opacity='0';setTimeout(go,120);});
  ...
  function paint(t){
   ...
   if(view!=='inside'&&!(bounds.width<600&&bounds.height/bounds.width>1.3)){const floor=lower&&lower.offsetParent?lower.getBoundingClientRect().top-bounds.top:bounds.height,cap=Math.max(css*.72,.8*Math.max(160,floor-top)/380);if(cap<css){css=cap;view720=bounds.width/css;vh=bounds.height/css;cx0=(720-view720)/2;}}   // tablets and desktops: pull the outside view back so the whole café and its garden read, never more than ~28%
