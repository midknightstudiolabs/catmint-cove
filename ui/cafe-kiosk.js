(function(root){
 ...
 root.CoveCafeView={open(a){
  ...
  panel.hidden=false;let previewFinish=null,confirmFinish=null,previewTier=null,confirmShop=false,previewDecor=null,confirmDecor=null;const floats=[];let workshop=false,recipeResult=null,lessonStep=0;let tasting='',previewEquipment=null,displayState=s;let stripOpen=(()=>{try{return localStorage.getItem('neo.cafe.strip')==='1';}catch(e){return false;}})();let lastTab=null,view='outside',tab=null,layout={k:0,cx0:0,offset:0},frame=0,last=0,timer;const actors=a.actors();stopActive=()=>{cancelAnimationFrame(frame);clearInterval(timer);};
  ...
  function render(){
   ...
   panel.classList.toggle('cc-locked',!s.unlocked);const needs=new Set(s.menu.length?E.recipes.filter(r=>s.menu.includes(r.id)).flatMap(r=>Object.keys(r.inputs)):['coffee','catmint','honey']);const goal=s.unlocked?E.nextGoal(g):null,goalHtml=goal?`<button class="cc-goal${g.shells>=goal.cost?' ready':''}" data-goal="${goal.id}" aria-label="Next goal: ${goal.label}, ${goal.cost.toLocaleString()} Shells"><span>Next: ${goal.label}</span><i style="--p:${Math.round(Math.min(1,g.shells/goal.cost)*100)}%"></i><b>${Math.floor(Math.min(g.shells,goal.cost)).toLocaleString()} / ${goal.cost.toLocaleString()}</b></button>`:'';const goalAffordable=!!goal&&g.shells>=goal.cost;const spId=E.special?E.special(s):null,spRecipe=spId&&E.recipes.find(r=>r.id===spId),specialHtml=spRecipe?`<p class="cc-special">★ Today’s special: ${E.displayName(s,spRecipe)} · pays ${E.price(s,spRecipe)} Shells</p>`:'';const rdy=ready(),ripeChip=rdy.count?`<button class="cc-pip cc-ripe" data-ripe aria-label="${rdy.count} patches ripe in the Garden"><span>${ripeLabel(rdy.count)}</span></button>`:'';const pips=ripeChip+[...needs].map(k=>{const n=g.homestead.stock[k]||0,low=n<(E.recipes.filter(r=>s.menu.includes(r.id)).reduce((m,r)=>Math.max(m,r.inputs[k]||0),1));return `<button class="cc-pip${low?' low':''}" data-pip="${k}" aria-label="${E.ingredients[k].name}: ${n} in pantry${low?', running low':''}"><span>${E.ingredients[k].name}</span><b>${n}</b></button>`;}).join('');const anyLow=s.unlocked&&pips.includes(' low');panel.innerHTML=`<canvas aria-label="${view==='inside'?'Behind the counter, customers ordering at the window':'Seaside café kiosk'}" width="720" height="380"></canvas><div class="cc-top"><header class="cc-head"><button class="btn cc-back" data-close aria-label="Return to Cove">‹ Cove</button><h2 class="cc-title"></h2><span class="cc-head-end"></span></header>${s.unlocked?`<div class="cc-live${stripOpen?'':' compact'}"><div class="cc-live-row"><b class="cc-shells">${Math.floor(g.shells).toLocaleString()} Shells</b><span>${E.rating(s)?'★ '+E.rating(s).toFixed(1)+' · ':''}${s.served} served</span><button class="cc-more" type="button" aria-expanded="${stripOpen}" aria-label="${stripOpen?'Show less':'Show more'}"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 9l6 6 6-6"/></svg></button></div><div class="cc-pips${(anyLow||rdy.count)?' cc-attn':''}" role="group" aria-label="Pantry stock">${pips}</div><p class="cc-status" role="status"></p>${specialHtml}<div class="cc-cta-row"></div>${goalHtml}</div>`:`<div class="cc-live cc-live-lite"><b class="cc-shells">${Math.floor(g.shells).toLocaleString()} Shells</b></div>`}</div><div class="cc-switch" aria-label="Café view"><button aria-pressed="${view==='outside'}" data-view="outside">Outside</button><button aria-pressed="${view==='inside'}" data-view="inside">Inside</button></div>`;
