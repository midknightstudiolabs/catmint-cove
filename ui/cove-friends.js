...
async function join(){const body=page('Your name in the Cove','Friends will see this name beside your invitation.');const name=input(body,'Cove name','text',24);name.value=api.name().slice(0,24);body.append(button('Meet your friends',async()=>{await rpc('join',{name:name.value.trim()});await home();dialog.querySelector('[role=status]').textContent='Your code is ready. Save a recovery key in Account & privacy to protect your circle.';},true));}
async function home(){
 ...
 const code=el('section',null,'friends-card friends-code');code.append(el('span','My code · share this with a friend','muted'),el('strong',data.profile.code.match(/.{1,4}/g).join(' ')),button('Copy code',async()=>{await navigator.clipboard.writeText(data.profile.code);dialog.querySelector('[role=status]').textContent='My code copied. Send it to your friend.';}));if(navigator.share)code.append(button('Share code',async()=>{try{await navigator.share({title:'Cove Friends',text:'Visit my Catmint Cove! My friend code is '+data.profile.code+'. Add it in Journal → Cove Friends.'});}catch(e){if(e.name!=='AbortError')throw e;}}));body.append(code);
 body.append(button('Add a friend',()=>addFriend(data.profile.code),true));
 const sharing=el('section',null,'friends-card');sharing.append(el('strong','Your shared Cove'),el('p',data.shared_at?'Shared '+new Date(data.shared_at).toLocaleDateString():'Only you can see your Cove until you share it.','muted'),button(data.shared_at?'Update shared picture':'Share my Cove',preview,true));if(data.shared_at)sharing.append(button('Stop sharing',async()=>{await rpc('unpublish');await home();}));const shareDetails=el('details',null,'friends-settings');shareDetails.append(el('summary','My shared Cove'),sharing);body.append(shareDetails);
 body.append(el('h3','Your circle'));let lastGroup='';
 ...
 const consent=el('label',null,'friends-consent'),check=el('input');check.type='checkbox';check.checked=localStorage.getItem(CONSENT)==='yes';check.onchange=()=>localStorage.setItem(CONSENT,check.checked?'yes':'no');consent.append(check,document.createTextNode('Share basic feature counts to help improve the game. No messages, names or game saves are included.'));const settings=el('details',null,'friends-settings');settings.append(el('summary','Account & privacy'));body.append(settings);settings.append(button('Refresh friends',home),consent,button('Protect my Friends account',protectAccount),button('Blocked players',blockedPlayers),button('Recover an existing account',recoverAccount),button('Cove backups',backups),button('Recovery & support',recoveryHelp));
}
function addFriend(ownCode){const data={profile:{code:ownCode}},body=page('Add a friend','Ask your friend for their code. Enter it below.');
 const invite=el('form',null,'friends-invite');const field=input(invite,"Friend’s code",'text',16);field.placeholder='ABCD 1234 EFGH';field.autocomplete='off';field.autocapitalize='characters';field.spellcheck=false;const send=button('Find friend',async()=>{const value=field.value.replace(/[^a-zA-Z0-9]/g,'').toUpperCase();if(!/^[A-Z0-9]{12}$/.test(value))throw Error('Enter the 12-character friend code.');if(value===data.profile.code)throw Error('That is your own code. Ask your friend for theirs.');const peer=await rpc('preview',{code:value});const review=page('Is this your friend?','Only an accepted invitation allows you to visit each other.');review.append(el('strong',peer.name),el('p',peer.code.match(/.{1,4}/g).join(' '),'muted'),button('Send invitation',async()=>{await rpc('request',{code:peer.code});await home();dialog.querySelector('[role=status]').textContent='Invitation sent. Your friend must accept before you can visit.';},true),button('Cancel',home));},true);invite.append(send);invite.onsubmit=e=>{e.preventDefault();send.click();};body.append(invite,button('Back to friends',home));
}
