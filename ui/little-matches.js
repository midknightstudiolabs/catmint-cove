...
function start(n,favour,cat){clear();if(!allDone())n=nextLevel();lastAction=Date.now();mood='watching';misses=0;round={level:n,deck:build(n),matched:[],turns:0,used:false,favour,cat,last:[],finished:false};round.analyticsRound=window.CoveAnalytics?.start("Little Matches");persist();play();openingLook();}
function startEndless(favour,cat){if(state().endless.rounds>=1000){endlessSetup();return}clear();lastAction=Date.now();mood='watching';misses=0;round={endless:true,deck:buildEndless(),matched:[],turns:0,used:false,favour,cat,last:[],finished:false};round.analyticsRound=window.CoveAnalytics?.start("Little Matches");persist();play();openingLook();}
...
function complete(){if(round.finished)return;window.CoveAnalytics?.complete("Little Matches",round.analyticsRound);round.finished=true;walkUntil=performance.now()+850;
