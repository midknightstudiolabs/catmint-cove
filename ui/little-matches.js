...
function home(){clear();dlg.replaceChildren();const top=el('div','lm-top');top.append(el('div','lm-kicker','LITTLE MATCHES · 30 LEVELS'));dlg.append(top,el('h2','','Little Matches'),el('p','lm-intro','Familiar faces. Favourite things. Your favourite company.'));
 ...
 dlg.append(el('p','lm-note',allDone()?'All 30 levels complete. Every level is yours to revisit.'+(endlessRounds?' Endless rounds played: '+endlessRounds+'.'+(nextEndlessReward?' Next reward at round '+nextEndlessReward.round+'.':''):''):'Complete each level to continue. Earn an accessory every five levels. No timer, lives, or entry cost.'));
 ...
 if(!allDone())dlg.append(el('p','lm-note','Endless mode unlocks at Level 30 — a fresh board every round, a few pearls every 20 rounds, and a new accessory every 50, all the way to round 1000.'));
}
